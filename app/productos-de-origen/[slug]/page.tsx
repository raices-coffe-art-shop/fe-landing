...
import { getCatalogItemBySlug, getLegacyCatalogDestinationBySlug, getRelatedCatalogItems } from "@/sanity/lib/catalog";
...
export async function generateMetadata({ params }: ProductPageProps): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `/productos-de-origen/${product.slug}` },
    openGraph: {
      ...baseOpenGraph,
      url: `/productos-de-origen/${product.slug}`,
      title: product.seo?.title || product.title,
      description: product.seo?.description || product.shortDescription,
...
export default async function ProductPage({ params }: ProductPageProps) {
  ...
  if (!product) {
    ...
    if (destination === "galeria-de-arte") {
      const legacyArtItem = await getArtItemBySlug(slug);
      redirect(legacyArtItem ? `/galeria-de-arte/${legacyArtItem.slug}` : "/galeria-de-arte");
    }
    notFound();
  }
    ...
    <>
      ...
      <JsonLd
        data={breadcrumbJsonLd([
          ...
          { name: "Productos de Origen", url: absoluteUrl("/productos-de-origen") },
          { name: product.title, url: absoluteUrl(`/productos-de-origen/${product.slug}`) },
        ])}
      />
      <SiteHeader />
        ...
        <section className="detail-hero catalog-detail-hero">
          ...
          <div className="page-shell detail-hero-grid catalog-detail-grid">
            <div className="catalog-detail-copy">
              <Link href="/productos-de-origen" className="back-link">← Volver a Productos de Origen</Link>
